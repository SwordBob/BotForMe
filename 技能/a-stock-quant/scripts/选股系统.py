...
优化点：
1. 精简股票池（核心龙头25只），减少总请求量
2. 增强连接稳定性（重试机制 + 指数退避sleep）
3. 批量获取数据，单次登录获取多只股票
4. 稳定扫描进度显示
"""
...
# ---- 精选股票池（核心龙头25只，减少冗余）----
# 去掉了重复的 sh.600028（中国石化出现2次）和 sh.600887（伊利股份出现2次）
# 保留各行业代表性龙头
STOCKS = [
    # 白酒消费
    ('sh.600519', '贵州茅台'), ('sh.000858', '五粮液'), ('sh.000568', '泸州老窖'),
    # 银行（今日扫描到的建设银行、中国银行保留）
    ('sh.601939', '建设银行'), ('sh.601988', '中国银行'), ('sh.600036', '招商银行'),
    # 保险券商
    ('sh.601318', '中国平安'), ('sh.600030', '中信证券'),
    # 消费医药
    ('sh.603288', '海天味业'), ('sh.600276', '恒瑞医药'), ('sz.300760', '迈瑞医疗'),
    # 科技制造
    ('sh.300750', '宁德时代'), ('sz.002594', '比亚迪'), ('sz.000333', '美的集团'),
    ('sz.002415', '海康威视'), ('sz.002475', '立讯精密'),
    # 芯片科技
    ('sh.688981', '中芯国际'), ('sh.688111', '金山办公'),
    # 电力新能源
    ('sh.601012', '隆基绿能'), ('sh.600900', '长江电力'),
    # 互联网金融
    ('sz.300059', '东方财富'),
    # 基建周期
    ('sh.601668', '中国建筑'), ('sh.601857', '中国石油'),
    # ETF（作为大盘参照）
    ('sh.510300', '沪深300ETF'),
]


def _retry_connect(max_retries=3, base_sleep=1.0):
    """带指数退避的连接重试"""
...
def _retry_query(code, days=300, max_retries=3):
    """带重试的数据查询，每次重试重新登录"""
    for attempt in range(max_retries):
        try:
            # 每次重试都重新登录（连接可能已断开）
            if not _retry_connect():
                continue
...
def 筛选(df, adx_thresh=30):
    """是否满足：MA5>MA20>MA60 且 ADX>thresh"""
    if df is None or len(df) < 65:
        return False, None
    r = df.iloc[-1]
    if np.isnan(r['ma5']) or np.isnan(r['ma20']) or np.isnan(r['ma60']):
        return False, None
    cond = (r['ma5'] > r['ma20'] > r['ma60']) and (r['adx'] > adx_thresh)
    return cond, r
...
def 趋势回测(df, fee=0.001):
    ...
    for i in range(1, len(df)):
        ...
        if cur and not prev and pos == 0:
            ...
            cash += pos * close[i] * (1-fee)
            pos = 0
        prev = cur
    if pos > 0:
        cash += pos * close[-1] * (1-fee)
...
''')
    ...
    # ---- 阶段1：扫描强势股 ----
    print('阶段1：扫描强势股（ADX>30）...')
    screened_strict = []

    for i, (code, name) in enumerate(pool):
        df = _retry_query(code, days=300)
        if df is None:
            time.sleep(0.3)
            continue
        df = 计算指标(df)
        cond, r = 筛选(df, adx_thresh=30)
        if cond:
            screened_strict.append({
                'code': code, 'name': name,
        ...
        # 进度显示
        pct = (i+1) / len(pool) * 100
        bar_len = 30
        filled = int(bar_len * (i+1) / len(pool))
        bar = '█' * filled + '░' * (bar_len - filled)
        print(f'\r  [{bar}] {i+1}/{len(pool)} ({pct:.0f}%) | 强势股 {len(screened_strict)} 只', end='', flush=True)
        time.sleep(0.25 + random.uniform(0, 0.1))

    print()  # 换行

    # ---- 阶段2：降低阈值重扫（ADX>25）----
    screened = screened_strict
    if not screened:
        print('\n未找到 ADX>30 的股票，降低阈值到 25...')
        screened_relaxed = []
        for i, (code, name) in enumerate(pool):
            df = _retry_query(code, days=300)
            if df is None:
            ...
            print(f'\r  降阈值扫描 {i+1}/{len(pool)} | 找到 {len(screened_relaxed)} 只', end='', flush=True)
            time.sleep(0.25 + random.uniform(0, 0.1))
        print()
        screened = screened_relaxed
    ...
    if screened:
        ...
        if results:
            ...
            print(f'\n  🔥 ADX最强（趋势最确定）：')
            for s in screened[:3]:
                print(f'     {s["name"]} ADX={s["adx"]}')
    else:
        print('\n未筛选到任何股票（ADX>25阈值下也无）')
